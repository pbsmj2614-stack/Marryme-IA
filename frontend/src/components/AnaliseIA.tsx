...
function NotaGeral({ nota }: { nota: number }) {
  ...
  const bg    = nota >= 7 ? "bg-green-50 border-green-200" : nota >= 5 ? "bg-yellow-50 border-yellow-200" : "bg-red-50 border-red-200";
  const label = nota >= 7 ? "Boa performance" : nota >= 5 ? "Precisa melhorar" : "Performance crítica";
  return (
    ...
    return (
      <div className="bg-gradient-to-br from-brand-50 to-purple-50 border border-brand-200 rounded-xl p-6 text-center">
        ...
        <p className="text-xs text-gray-500 mb-4 max-w-xs mx-auto">
          Diagnóstico completo dos seus dados Meta Ads com recomendações estratégicas personalizadas.
        </p>
        {erro && (
      ...
      <div className="p-5">
        ...
        {tab === "resumo" && (
          <div className="space-y-4">
            <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap">{analise.resumo_executivo}</p>
            {analise.mensagem_para_cliente && (
              <div className="bg-brand-50 border border-brand-200 rounded-xl p-4">
                <p className="text-xs font-bold text-brand-600 uppercase tracking-wide mb-1">Mensagem para o cliente</p>
                <p className="text-sm text-brand-800 leading-relaxed">{analise.mensagem_para_cliente}</p>
              </div>
            )}
          </div>
        )}
            ...
            {(analise.analise_campanhas ?? []).map((c, i) => (
              <div key={i} className="border border-gray-100 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-1.5">
                  <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${statusCampanhaCls(c.status)}`}>
        ...
        {tab === "diagnostico" && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            {analise.diagnostico && ([
              ...
              { key: "riscos",         label: "Riscos",         cls: "border-yellow-200 bg-yellow-50",dot: "bg-yellow-500" },
            ] as const).map(({ key, label, cls, dot }) => (
        ...
        {tab === "recomendacoes" && (
          <div className="space-y-3">
            {(analise.recomendacoes ?? []).map((r, i) => (
              <div key={i} className="border border-gray-100 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-1.5">
                  <span className={`text-xs font-semibold px-2 py-0.5 rounded-full capitalize ${prioridadeCls(r.prioridade)}`}>
                    {r.prioridade}
                  </span>
                ...
                {r.impacto_esperado && (
                  <p className="text-xs text-brand-600 mt-1.5 font-medium">Impacto: {r.impacto_esperado}</p>
                )}
              </div>
            ))}
          </div>
        ...
        {tab === "pauta" && (
          <div>
            <div className="flex items-center justify-between mb-3">
              <p className="text-xs font-bold text-gray-400 uppercase tracking-wide">Pauta da reunião</p>
              <button
                onClick={copiarPauta}
            ...
            {(analise.proximos_passos ?? []).map((p, i) => (
              ...
              >
                ...
                <div className="flex-1">
                  <span className={`text-xs font-semibold px-2 py-0.5 rounded-full mr-2 ${prazoCls(p.prazo)}`}>
                    {prazoLabel(p.prazo)}
                  </span>
                  <span className={`text-sm text-gray-700 ${passosFe[i] ? "line-through text-gray-400" : ""}`}>
                    {p.acao}
                  </span>
                </div>
              </div>
