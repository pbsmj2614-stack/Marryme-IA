    ...
    <>
      ...
      <div className="max-w-[800px] mx-auto px-8 py-10 font-sans text-gray-900">
        ...
        <div className="flex items-start justify-between mb-8 pb-6 border-b-2 border-gray-900">
          <div>
            <p className="text-xs font-bold tracking-widest text-gray-400 uppercase mb-1">Relatório de Campanha</p>
          ...
          <>
            ...
            {sc && score !== null && (
              ...
              >
                ...
                <div>
                  <p className="text-xl font-bold" style={{ color: sc.color }}>{sc.label}</p>
                  <p className="text-sm text-gray-600 mt-1">
                    Health Score calculado com base em CTR, frequência e CPM das campanhas ativas no período.
                  </p>
                </div>
              </div>
            )}
            ...
            {/* KPIs */}
            <h2 className="text-xs font-bold tracking-widest text-gray-400 uppercase mb-3">Métricas do período</h2>
                ...
                <table className="w-full text-sm mb-6" style={{ borderCollapse: "collapse" }}>
                  <thead>
                    <tr style={{ borderBottom: "2px solid #e5e7eb" }}>
                      {["Campanha", "Impressões", "Cliques", "CTR", "CPM", "Gasto", "Resultados"].map((h) => (
                        <th key={h} className={`py-2 text-[10px] font-bold text-gray-400 uppercase tracking-wide ${h === "Campanha" ? "text-left pr-3" : "text-right px-2"}`}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {(campanhas as CampanhaInsight[]).map((c, i) => (
                      <tr key={c.campaign_id} style={{ borderBottom: "1px solid #f3f4f6", backgroundColor: i % 2 ? "#fafafa" : "white" }}>
                        <td className="py-2 pr-3 text-gray-800 font-medium max-w-[200px] truncate" style={{ maxWidth: 200 }}>
                          {c.campaign_name}
                        </td>
                        ...
                        <td className="py-2 px-2 text-right font-medium" style={{ color: c.ctr >= 1 ? "#16a34a" : c.ctr >= 0.5 ? "#ca8a04" : "#dc2626" }}>
                          {fmtPct(c.ctr)}
                        </td>
                        <td className="py-2 px-2 text-right text-gray-600">{fmtBRL(c.cpm)}</td>
                        <td className="py-2 px-2 text-right text-gray-700 font-medium">{fmtBRL(c.spend)}</td>
                        <td className="py-2 pl-2 text-right font-bold text-gray-900">{fmt(c.results)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="bg-gray-50 rounded-xl p-5 border border-gray-100">
              <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-3">Interpretação do Health Score</h3>
              <div className="grid grid-cols-3 gap-4 text-sm">
                <div>
                  <p className="font-semibold text-green-700 mb-1">70–100 · Saudável</p>
                  <p className="text-gray-600 text-xs">CTR alto, frequência baixa e CPM eficiente. Campanha performando bem.</p>
                </div>
                <div>
                  <p className="font-semibold text-yellow-700 mb-1">40–69 · Atenção</p>
                  <p className="text-gray-600 text-xs">Pontos de melhoria identificados. Revisão de segmentação ou criativo recomendada.</p>
                </div>
                <div>
                  <p className="font-semibold text-red-700 mb-1">0–39 · Em Risco</p>
                  <p className="text-gray-600 text-xs">CTR baixo e/ou frequência alta. Ação imediata necessária para evitar desperdício.</p>
                </div>
              </div>
            </div>
          </>
