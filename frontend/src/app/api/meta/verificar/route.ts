...
export async function GET(req: NextRequest) {
  ...
  if (META_APP_ID && META_APP_SECRET) {
    try {
      ...
      const d = json.data;
      if (d && !json.error) {
        resultado.token_expira_em =
          d.expires_at !== undefined
            ? d.expires_at === 0
              ? "nunca"
              : new Date(d.expires_at * 1000).toLocaleString("pt-BR")
            : "desconhecido";
  ...
  try {
    const res = await fetch(
      `${META_BASE}/me/adaccounts?fields=id,name,account_status&access_token=${token}`
    );
    const json = (await res.json()) as {
    ...
    resultado.contas_erro = String(e);
  }
  ...
  if (accountId) {
    ...
    const contasAcessiveis = resultado.contas_acessiveis as
    ...
    if (contaNaLista) {
      ...
      try {
        const res = await fetch(
          `${META_BASE}/act_${accountId}?fields=id,name,account_status&access_token=${token}`
        );
        const json = (await res.json()) as {
          id?: string;
          name?: string;
        ...
        if (json.error) {
          ...
          resultado.conta_status =
            json.account_status === 1 ? "Ativa" : `Inativa (${json.account_status})`;
        }
      } catch (e) {
        resultado.conta_erro = String(e);
      }
    }
  }
  ...
  const ok = resultado.token_valido === true && (!accountId || resultado.conta_acessivel === true);
